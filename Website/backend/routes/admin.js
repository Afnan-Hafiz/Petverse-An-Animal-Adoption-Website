...
router.get('/posts', authenticate, requireAdmin, async (req, res) => {
  try {
    const { type, animal_type_id, page = 1, limit = 50 } = req.query;
    ...
    let sql = `
      SELECT
        ...
        bs.category, bs.price, m.expire_date,
    ...
    if (animal_type_id) { sql += ' AND ad.animal_type_id = ?'; params.push(animal_type_id); }
