...
router.get('/', async (req, res) => {
  try {
    const { type, animal_type_id, category, search, page = 1, limit = 20 } = req.query;
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      try {
        ...
        const decoded = require('jsonwebtoken').verify(token, process.env.JWT_SECRET || 'animal_adoption_secret');
        currentUserId = decoded.account_id;
    ...
    let sql = `
      SELECT
        p.post_id, p.description, p.post_date, p.post_type,
        ...
        -- BuySell fields
        bs.buysell_id, bs.category, bs.price,
        m.expire_date,
        -- Moderation flag
        (SELECT COUNT(*) FROM Moderation m2 WHERE m2.post_id = p.post_id) AS moderated
        ${currentUserId ? `, (SELECT COUNT(*) FROM Wishlists w WHERE w.post_id = p.post_id AND w.user_id = ?) AS is_wishlisted` : ''}
      FROM Posts p
    ...
    // Non-admins can only see moderated posts
    if (!isAdmin) {
      sql += ' AND (SELECT COUNT(*) FROM Moderation m2 WHERE m2.post_id = p.post_id) > 0';
    }
    ...
    if (search && search.trim()) {
      sql += ` AND CONCAT_WS(' ', p.description, ad.animal_name, ad.location, at.name, a.name, bs.category) LIKE ?`;
    ...
    `;
    const countParams = [];
    if (!isAdmin) { countSql += ' AND (SELECT COUNT(*) FROM Moderation m2 WHERE m2.post_id = p.post_id) > 0'; }
    ...
    if (search && search.trim()) {
      countSql += ` AND CONCAT_WS(' ', p.description, ad.animal_name, ad.location, at.name, a.name, bs.category) LIKE ?`;
...
router.post('/', authenticate, async (req, res) => {
  ...
  try {
    ...
    const [postResult] = await conn.query(
      'INSERT INTO Posts (account_id, description, post_date, post_type) VALUES (?, ?, ?, ?)',
      [req.user.account_id, description || null, today, post_type]
    );
    ...
    if (post_type === 'Adoption') {
      ...
      const { category, expire_date, price } = req.body;
      if (!category || !['Medicine', 'Toys'].includes(category)) {
        await conn.rollback();
        return res.status(400).json({ message: 'category must be Medicine or Toys' });
      }

      const pval = price ? parseFloat(price) : 0.00;

      const [bsResult] = await conn.query(
        'INSERT INTO BuySell (post_id, category, price) VALUES (?, ?, ?)',
        [post_id, category, pval]
      );
